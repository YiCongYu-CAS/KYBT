%  Note that this section of code depends on the data generated by 
%  CodeSciPost8_step1.m. We ran this code on the server and then saved 
%  the data to H:\FEA_Dn_symmetry_plot_step1_data2345.mat 
%  to complete the plotting.
...
fig88 = figure(88); hold on;


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ax1 = subplot(2,2,1, 'Parent', fig88);
hold on;

load('H:\FEA_Dn_symmetry_plot_step1_data5.mat');
tNumber5 = size(t, 2);

cSolution = cWaveFuncGamma1{1};
vu = fn_Ext(p0, p0, cSolution, cgMatrix);
En = cSolution{1};
vu = vu/sqrt(real(vu'*m00*vu));
k0 = sqrt(En);

vth = linspace(0.05, pi/3-0.05, 50);
vDiss = zeros(1, length(vth));

for ith = 1:length(vth)
    th = vth(ith);
    [vkx, vky] = fn_generate_Kg(k0, th, 6);
    ...
    for ngp = 1:12
        for ng = 1:12
            ...
            Mgpg(ngp, ng) = fn_int_exp(p0, t0, dkx, dky);
    ...
    for ngp = 1:12
        ...
        vBg(ngp) = fn_int_exp_phi(p0, t0, vu, -kxp, -kyp);
        
    end
    
    Diss = vu'*m00*vu - vBg'*(Mgpg\vBg);
    
    vDiss(ith) = real(Diss);
    
    fprintf('finished ith = %d \n', ith);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
load('H:\FEA_Dn_symmetry_plot_step1_data4.mat');
tNumber4 = size(t, 2);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
load('H:\FEA_Dn_symmetry_plot_step1_data3.mat');
tNumber3 = size(t, 2);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
load('H:\FEA_Dn_symmetry_plot_step1_data2.mat');
tNumber2 = size(t, 2);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tf = ax1;
xticks([0, pi/6, pi/3]);  % 自定义横坐标刻度位置
xticklabels({'$0$', '$\pi/6$', '$\pi/3$'}); 
set(gca, 'TickLabelInterpreter', 'latex');% LaTeX 标签
% set(gca, 'YScale', 'log');
xlim(tf, [0,pi/3]);

xlabel(tf, '$\theta$', 'Interpreter', 'latex', 'FontSize', 20);
ylabel(tf, '$1-\vert \langle \Psi_\mathrm{BA} \vert \Psi_0 \rangle \vert$', 'Interpreter', 'latex', 'FontSize', 20);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ax2 = subplot(2,2,2, 'Parent', fig88);
hold on;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ax3 = subplot(2,2,3, 'Parent', fig88);
hold on;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ax4 = subplot(2,2,4, 'Parent', fig88);
hold on;
